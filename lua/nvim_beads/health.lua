--- Make sure `nvim_beads` will work as expected.
---
--- At minimum, we validate that the user's configuration is correct. But other
--- checks can happen here if needed.
---

local configuration_ = require("nvim_beads._core.configuration")
local logging_ = require("mega.logging")
local say_constant = require("nvim_beads._commands.hello_world.say.constant")
local tabler = require("nvim_beads._core.tabler")
local texter = require("nvim_beads._core.texter")

local _LOGGER = logging_.get_logger("nvim_beads.health")
...
---
---@param data nvim_beads.Configuration All of the user's fallback settings.
...
---
---@param command string A supported `nvim_beads` command. e.g. `"hello_world"`.
---@return string[] # All found issues, if any.
---
...
---
---@param data nvim_beads.LoggingConfiguration The user's logger settings.
...
function M.get_issues(data)
    if not data or vim.tbl_isempty(data) then
        data = configuration_.resolve_data(vim.g.nvim_beads_configuration)
    end
...
--- Make sure `data` will work for `nvim_beads`.
---
---@param data nvim_beads.Configuration? All extra customizations for this plugin.
---
function M.check(data)
    _LOGGER:debug("Running nvim-beads health check.")
    ...
    if vim.tbl_isempty(issues) then
        vim.health.ok("Your vim.g.nvim_beads_configuration variable is great!")
    end
