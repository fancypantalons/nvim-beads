...
function M.handle_existing_issue_save(bufnr, issue_id)
  ...
  if err then
    vim.notify(
      string.format('nvim-beads: Failed to fetch issue %s: %s', issue_id, err),
      vim.log.levels.ERROR
    )
    return
  end
  ...
  if not original_issue or not original_issue.id then
    vim.notify(
      string.format('nvim-beads: Invalid issue data for %s', issue_id),
      vim.log.levels.ERROR
    )
    return
  end
  ...
  if changes.parent == '' then
    ...
    if original_parent_id then
      ...
      if not remove_ok then
        vim.notify(
          string.format('nvim-beads: Failed to remove parent: %s', remove_cmd),
          vim.log.levels.ERROR
        )
        return
      end
  ...
  for i, cmd in ipairs(commands) do
    ...
    if exit_code ~= 0 then
      vim.notify(
        string.format(
          'nvim-beads: Command failed (exit %d): %s\nOutput: %s',
          exit_code,
          cmd,
          cmd_result
        ),
  ...
  if all_success then
    ...
    if not updated_issue or not updated_issue.id then
      vim.notify(
        string.format('nvim-beads: Invalid updated issue data for %s', issue_id),
        vim.log.levels.ERROR
      )
      return
    end
